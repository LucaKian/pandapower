...
def _add_measurements_to_ppci(net, ppci, zero_injection):
    ...
    v_dg_measurements = meas_bus[(meas_bus.measurement_type == "v_degree")]
    if len(v_dg_measurements):
        bus_positions = map_bus[v_dg_measurements.element.values.astype(int)]
        bus_append[bus_positions, VA] = v_dg_measurements.value.values
        bus_append[bus_positions, VA_STD] = v_dg_measurements.std_dev.values
        bus_append[bus_positions, VA_IDX] = v_dg_measurements.index.values
...
def _build_measurement_vectors(ppci):
    """
    Building degree_measurement vector z, pandapower to ppci measurement mapping and covariance matrix R
